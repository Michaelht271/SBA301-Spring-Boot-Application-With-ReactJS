...
const JobRequestForm = ({
  ...
  // Auto-scroll to error message when error appears - works every time
  useEffect(() => {
    if (error && errorRef.current && formRef.current) {
      // Scroll the parent container (popup content area) to top
  ...
  return (
    ...
    >
      {error && (
        <div ref={errorRef} className="p-3 bg-red-50 border border-red-200 text-red-700 text-sm rounded-lg flex items-start gap-2">
