...
const resolveApiGateway = () => {
    const rawApiGateway = import.meta.env.VITE_API_GATEWAY;
    const cleanedApiGateway = cleanEnvValue(rawApiGateway);

    // Dev: call local gateway directly. Prod: keep relative path for same-origin deployments.
    const defaultApiGateway = import.meta.env.DEV
        ? "http://localhost:8080/api/v1"
        : "/api/v1";

    if (!cleanedApiGateway) {
        if (import.meta.env.DEV) {
            console.warn(`VITE_API_GATEWAY không được set. Dùng mặc định: ${defaultApiGateway}`);
        }
        return defaultApiGateway;
    }

    // Remove trailing slash to avoid duplicated slashes when appending endpoint paths.
    return cleanedApiGateway.replace(/\/+$/, '');
};
