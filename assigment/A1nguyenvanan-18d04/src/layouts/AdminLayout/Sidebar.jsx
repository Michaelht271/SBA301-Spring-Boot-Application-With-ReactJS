...
const Sidebar = ({ user }) => {
  // Check if the user is an admin. Handle string and number roles, and be case-insensitive.
  const isAdmin = user && (
    (typeof user.accountRole === 'string' && user.accountRole.toLowerCase() === 'admin') || 
    user.accountRole === 1
  );
  ...
  return (
    <div className="bg-dark border-right" id="sidebar-wrapper" data-bs-theme="dark">
      ...
      <Nav className="flex-column list-group list-group-flush">
        ...
        <Nav.Link as={NavLink} to="/users" className="list-group-item list-group-item-action">Users Management</Nav.Link>
